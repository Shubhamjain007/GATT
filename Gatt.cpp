...
state=(random(cross_over_rate))%2;
         ...
         for (cur_room = 0; cur_room<room_constraints.num_of_rooms;cur_room++)
             {
              if (state==1) 
              {
               ...
               if ((random(cross_over_rate))%2==0)
                {
                 state = (random(solution_colony.population_size))%2;
                 }
               }
               else {
...
{ temp=test_tube -> cost;
 cout<<"      .cost=@"<<(temp/100)<<((test_tube->cost)%100)/10<<temp%10;
 fputn(temp,graph);
 if(test_tube -> cost==0)
 { happy=happiness1(test_tube);
   
 if(happy<0)
 {happy=happy*(-1);fputs("-",graph);fputn(happy,graph);  }
 else
 fputn(happy,graph);
} 
 curr_ptr = solution_colony.first_time_table;
